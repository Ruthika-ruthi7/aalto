...
async function checkAdminUsers() {
  try {
    ...
    // Show existing users
    const [userList] = await connection.query('SELECT User_id, User_name, role FROM admin_users LIMIT 5');
    if (userList.length > 0) {
      console.log('\nExisting admin users:');
      userList.forEach(user => {
        console.log('  - ID:', user.User_id, '| Username:', user.User_name, '| Role:', user.role);
      });
    }
